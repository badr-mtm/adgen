...
const Dashboard = () => {
  ...
  useEffect(() => {
    const checkAuthAndFetch = async () => {
      ...
      try {
        ...
        const {
          data: brands
        } = await supabase.from("brands").select("*").eq("user_id", session.user.id).limit(1);
        ...
        if (campaigns) {
          ...
          if (campaigns.length > 0) {
            setScheduledSpots(campaigns.slice(0, 3).map((c, i) => ({
              id: c.id,
          ...
          if (campaigns.length > 0) {
            setComplianceItems(campaigns.slice(0, 3).map((c, i) => ({
              id: c.id,
              campaignTitle: c.title,
  ...
  return <DashboardLayout>
      <motion.div className="p-6 space-y-6 max-w-7xl mx-auto" initial={{
      opacity: 0
    }} animate={{
      opacity: 1
    }} transition={{
      duration: 0.3
        ...
        <ScrollReveal direction="down" duration={0.3}>
          <div className="flex items-center justify-between">
            ...
            <Button onClick={() => navigate("/create")} size="lg" className="bg-primary text-primary-foreground hover:bg-primary/90 h-12 px-6 shadow-lg shadow-primary/20">
        ...
        {!emailVerified && userEmail && <ScrollReveal direction="down" duration={0.35}>
            <EmailVerificationBanner email={userEmail} isVerified={emailVerified} />
          </ScrollReveal>}

        {/* Brand setup prompt (optional) */}
        {!brandProfile && <ScrollReveal direction="down" duration={0.4}>
            <BrandSetupPrompt />
          </ScrollReveal>}
