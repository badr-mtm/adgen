...
const Create = () => {
  ...
  const handleStrategyContinue = async (strat: AIStrategy, length: string) => {
    ...
    setConcepts([
      { id: "1", title: "Emotional Journey", description: "A heartfelt story that connects with viewers on a personal level", scenes: [{ number: 1, description: "Opening hook", duration: "3s" }, { number: 2, description: "Problem reveal", duration: "5s" }, { number: 3, description: "Solution", duration: "5s" }, { number: 4, description: "CTA", duration: "2s" }], tone: "Warm", style: "Cinematic", bestFor: "awareness" },
      { id: "2", title: "Fast-Paced Impact", description: "Quick cuts and dynamic visuals for maximum recall", scenes: [{ number: 1, description: "Bold opener", duration: "2s" }, { number: 2, description: "Product showcase", duration: "6s" }, { number: 3, description: "Benefits", duration: "5s" }, { number: 4, description: "Strong CTA", duration: "2s" }], tone: "Energetic", style: "Modern", bestFor: "recall" },
      { id: "3", title: "Repetition Power", description: "Memorable branding through strategic repetition", scenes: [{ number: 1, description: "Brand intro", duration: "3s" }, { number: 2, description: "Message 1", duration: "4s" }, { number: 3, description: "Message 2", duration: "4s" }, { number: 4, description: "Brand outro", duration: "4s" }], tone: "Confident", style: "Bold", bestFor: "frequency" }
    ]);
  ...
  const handleConfirmationContinue = async () => {
    ...
    setSimulation({
      ...
      estimatedCPM: 12.50,
      airingSchedule: [{ daypart: "Prime Time", percentage: 45 }, { daypart: "Fringe", percentage: 35 }, { daypart: "Daytime", percentage: 20 }],
      networkBreakdown: [{ network: "ABC", impressions: 3200000 }, { network: "NBC", impressions: 2800000 }, { network: "CBS", impressions: 2500000 }, { network: "FOX", impressions: 2000000 }],
      riskWarnings: []
    });
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      let { data: brands } = await supabase.from("brands").select("id").eq("user_id", session.user.id).limit(1);
      let brandId = brands?.[0]?.id;
      
      if (!brandId) {
        const { data: newBrand } = await supabase.from("brands").insert({ user_id: session.user.id, name: "My Brand" }).select().single();
        brandId = newBrand?.id;
      }
      ...
      toast({ title: "Campaign Published!", description: "Your TV campaign has been created successfully." });
    } catch (error: any) {
  ...
  return (
    <DashboardLayout>
      ...
      <StrategyConfirmationModal open={modalStep === "confirmation"} onOpenChange={(open) => !open && handleCloseModals()} onBack={() => setModalStep("production")} onContinue={handleConfirmationContinue} aiRecommendations={{ budgetAllocation: 70, flightingPattern: "pulsing", deliveryTiming: "prime", placementStrategy: "balanced", frequencyCap: 5 }} />
