...
const defaultAdFormats: AdFormatSpend[] = [
  { format: "Pre-Roll", icon: SkipForward, amount: 9840, percentage: 38, cpm: "$31.20", cpmTrend: "down" },
  { format: "Mid-Roll", icon: Play, amount: 8620, percentage: 33, cpm: "$28.60", cpmTrend: "down" },
  { format: "Pause Ads", icon: Pause, amount: 4280, percentage: 16, cpm: "$18.40", cpmTrend: "up" },
  { format: "Title Sponsorship", icon: Bookmark, amount: 3410, percentage: 13, cpm: "$42.80", cpmTrend: "flat" },
];

const defaultTopTitles: TopTitle[] = [
  { title: "Stranger Things S5", vcr: "97.2%", impressions: "2.4M" },
  { title: "Wednesday S2", vcr: "96.8%", impressions: "1.9M" },
  { title: "Squid Game S3", vcr: "95.4%", impressions: "2.1M" },
  { title: "The Night Agent S3", vcr: "94.1%", impressions: "1.2M" },
];
...
export function SpendIntelligencePanel(props: Partial<SpendIntelligencePanelProps>) {
  ...
  return (
    <div className="bg-card/40 border border-border/50 rounded-2xl p-6 h-full space-y-5 backdrop-blur-sm">
      ...
      <div className="space-y-3">
        <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Spend by Ad Format</h4>
        {data.adFormats.map((item, i) => {
            ...
            >
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1">
                    <span className="text-[10px] text-muted-foreground">{item.cpm}</span>
                    {item.cpmTrend === "down" && <TrendingDown className="h-3 w-3 text-emerald-500" />}
                    {item.cpmTrend === "up" && <TrendingUp className="h-3 w-3 text-red-500" />}
                  </div>
              ...
              <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                <motion.div 
                  ...
                  className={`h-full ${formatBarColors[i % formatBarColors.length]} rounded-full`} 
                />
              </div>
            </motion.div>
          );
