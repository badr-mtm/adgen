              ...
              {overlaySettings.title?.enabled && (
                <div className="space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="space-y-2">
                    ...
                    <Textarea
                      value={overlaySettings.title.text}
                      onChange={(e) => updateTitle({ text: e.target.value })}
                      placeholder="e.g. Modern Luxury Living&#10;Enter your tagline here"
                      className="bg-muted/30 border-border text-foreground focus:border-primary/50 resize-none min-h-[100px]"
                    />
                  </div>

                  <div className="space-y-2">
                    <Label className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">Position</Label>
                    <RadioGroup value={overlaySettings.title.position} onValueChange={(val) => updateTitle({ position: val as any })}>
                      <div className="grid grid-cols-2 gap-2">
                        {['top-left', 'top-right', 'bottom-left', 'bottom-right'].map((pos) => (
                          <div key={pos} className="flex items-center space-x-2 border border-border rounded-md p-2 hover:bg-accent cursor-pointer has-[:checked]:bg-primary/10 has-[:checked]:border-primary/30">
                            <RadioGroupItem value={pos} id={`title-${pos}`} className="text-primary border-border" />
                            <Label htmlFor={`title-${pos}`} className="text-[10px] cursor-pointer text-foreground uppercase">{pos.replace('-', ' ')}</Label>
                          </div>
                        ))}
                      </div>
                    </RadioGroup>
