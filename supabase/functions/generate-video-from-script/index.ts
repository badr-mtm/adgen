...
serve(async (req) => {
    ...
    try {
        ...
        console.log(`Using fal-ai/wan/v2.1/1.3b/text-to-video with prompt: ${prompt.substring(0, 100)}...`);

        const response = await fetch(`https://fal.run/fal-ai/wan/v2.1/1.3b/text-to-video`, {
            method: "POST",
            headers: {
